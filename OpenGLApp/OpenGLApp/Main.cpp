...
const float vibrationIntensity = 0.2f; // Intensità dello spostamento
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case GameState::MainMenu: {
			...
			if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
				// Verifica se il mouse è all'interno del bounding box di "Start"
				...
				// Verifica se il mouse è all'interno del bounding box di "Quit"
		...
		case GameState::GameOverMenu: {
			...
			if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
				...
				// Verifica se il mouse è all'interno del bounding box di "Restart"
...
int generateRandomObject() {
	static int count = 0;  // Tiene traccia di quante volte la funzione è stata chiamata
	...
	count++;
	if (count % 3 == 0) { // Ogni terzo numero è sempre 4
		return 4;
	}
	else {
		int num = dist(rng);
...
void processInput(GLFWwindow* window, int caller)
{
	...
	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) {
		if (collectedPowerupId == 0 && !powerupActive) {    // Invincibilità
			activePowerupId = 0;
...
void saveScore(int& collected, int& dropped, float& timePlayed) {
	...
	// Se esiste già un file JSON, caricalo
	...
	// Controlla se la run attuale è la migliore
